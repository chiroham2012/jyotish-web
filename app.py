...
def _check_password():
    ...
    # st.form を使うのは、入力欄で Enter を押しただけでも送信されるようにするため。
    # 素の text_input + button では Enter で再実行が走るだけでボタンが押されない扱いになり、
    # 正しい合言葉を入れても無反応のまま入れない（実際にこれで入れなくなった）。
    with st.form("_gate", clear_on_submit=False):
        pw = st.text_input("合言葉", type="password")
        submitted = st.form_submit_button("入る")
    if submitted:
        # 前後の空白は落とす。コピー＆ペースト時に紛れ込んで弾かれるのを防ぐ。
        if pw.strip() == str(required).strip():
            st.session_state["_authed"] = True
            st.rerun()
        else:
            st.error("合言葉が違います。前後に空白が入っていないかもご確認ください。")
    return False
...
# --- 出生地アトラス -----------------------------------------------------------
# 全国の市区町村（約1,900件）を cities_jp.py に同梱してある。以前は県庁所在地など
# 49件しか無く、柏崎市のような市は一覧に出ないため利用者が緯度経度を自分で調べて
# 手入力する必要があった。選択欄は文字を打つと絞り込めるので、「柏崎」と打てば
# 柏崎市が出る。海外生まれなど一覧に無い場合のために手入力も残してある。
MANUAL_ENTRY = "（この中にない・自分で入力する）"
CITIES = dict(CITIES_JP)
CITIES[MANUAL_ENTRY] = None
...
city = st.selectbox(
    ...
    list(CITIES.keys()),
    index=None,
    placeholder="市区町村名を入力して選んでください（例：柏崎）",
)
st.caption("市区町村名を入力すると絞り込めます。全国の市区町村（約1,900件）から選べます。")

if city is None:
    # 既定値を入れると、選び忘れに気づかないまま別人の土地で計算してしまうため、
    # 明示的に選ぶまで先へ進ませない。
    lat = lon = tz = None
    city_name = ""
elif CITIES[city] is None:
    st.caption("海外で生まれた方など、一覧に無い場合はこちらに入力してください。"
               "地図アプリで「（地名） 緯度経度」と検索すると調べられます。")
...
submitted = st.button("鑑定チャートを作成", type="primary", disabled=(city is None))
if city is None:
    st.caption("※ 出生地を選ぶと「鑑定チャートを作成」が押せるようになります。")
